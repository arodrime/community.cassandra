...
Covered: `cassandra.yaml`, `cassandra-env.sh`, `jvm-server.options`,
`jvm11-server.options`, `jvm17-server.options`,
`cassandra-rackdc.properties` and `logback.xml`, from stock Apache
Cassandra 5.0.9 (`conf/` at tag `cassandra-5.0.9`). Templates live in
`templates/<series>/`, picked from `cassandra_version` (only `50x` so far).
...
* `cassandra-env.sh`: `cassandra_log_dir` (defaults to `/var/log/cassandra`,
  ...
  `$CASSANDRA_HOME/logs`), `cassandra_heap_size` (empty = stock auto-sizing),
  `cassandra_max_direct_memory_size`, `cassandra_heap_dump_dir`,
  `cassandra_local_jmx`, `cassandra_jmx_port`, `cassandra_jmx_rmi_hostname`.
* `jvm11-server.options` / `jvm17-server.options`: G1 settings as
  ...
  `cassandra_jvm11_extra_options` and `cassandra_jvm17_extra_options` append
  extra lines.
...
The molecule scenario renders every file with defaults only and checks it
line for line against the stock 5.0.9 files in
`molecule/default/files/stock-5.0.9/`, then renders them again with
overrides and checks the changed lines.

Adding a Cassandra version
--------------------------

All templates except `cassandra.yaml.j2` are generated from the stock files
by `tools/gen_templates.py` (repo root), which fails if a line it replaces
is missing or duplicated in the new version:

    tools/gen_templates.py <cassandra-x.y.z>/conf roles/cassandra_config/templates/x.y
